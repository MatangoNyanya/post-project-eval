# -*- coding: utf-8 -*-
# Auto-generated from 40_assign_freedomrate.ipynb
# Cells are delimited with '# %%' markers.

# %% [markdown]
## freedomhouse rateを付与する

# %%
import pandas as pd
df = pd.read_csv("df_check_7.csv", index_col=0)
df

# %%
df_data = pd.read_csv("input/input_freedom_rate.csv")
...
df_long = df_data.melt(
    id_vars=['country_name_en', 'country'],
    var_name='Year',
    value_name='Value'
)
...
df_long = df_long[df_long['Value']!='#VALUE!']
# 結果確認
...
# %%
df_long = df_long.merge(df_country,on='country', how='left')
df_long['Value'] = pd.to_numeric(df_long['Value'], errors='coerce')
df_long = df_long.rename(columns={'Value': 'freedom_house_score'})
...
df_long = df_long.drop(columns=drop_col)
df_long
...
cond = (
    (~df['join_flg']) |
    (df['country'].isin(['トルコ','パレスチナ'])) | #トルコパレスチナだけ結合先なしのため補完
    (df['地域詳細'].isna()) |
    (
